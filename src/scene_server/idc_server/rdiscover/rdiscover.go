...
func (r *RegDiscover) Start() error {
	...
	if err := r.registerIdc(); err != nil {
		blog.Error("fail to register idc(%s), err:%s", r.ip, err.Error())
		return err
	}
...
func (r *RegDiscover) registerIdc() error {
	idcServInfo := new(types.IdcServInfo)

	idcServInfo.IP = r.ip
	idcServInfo.Port = r.port
	idcServInfo.Scheme = "http"
	if r.isSSL {
		idcServInfo.Scheme = "https"
	}

	idcServInfo.Version = version.GetVersion()
	idcServInfo.Pid = os.Getpid()

	data, err := json.Marshal(idcServInfo)
	if err != nil {
