...
import (
	...
	"configcenter/src/scene_server/idc_server/idc_service/actions/object"
...
func (cli *instAction) subCreateInst(forward *api.ForwardParam, req *restful.Request, defErr errors.DefaultCCErrorIf, targetInput map[string]interface{}, ownerID, objID string, isBatch bool, asstDes []api.ObjAsstDes, attDes []api.ObjAttDes) (int, interface{}, bool, error) {
	...
	ignorItems = append(ignorItems, common.BKIdcIDField)
		...
		parseChildFunc = func(child []manager.TopoInstRst) (int, interface{}, error) {
			...
			for _, instItem := range child {
				...
				case common.BKInnerObjIDPos:
					...
					rstOk, rstErr := hasHost(req, cli.CC.HostCtrl(), map[string][]int{common.BKPosIDField: []int{instItem.InstID}})
					if nil != rstErr {
				...
				case common.BKInnerObjIDRack:
					// check wether it can be delete
					rstOk, rstErr := hasHost(req, cli.CC.HostCtrl(), map[string][]int{common.BKRackIDField: []int{instItem.InstID}})
					if nil != rstErr {
						blog.Error("failed to check app wether it has hosts, error info is %s", rstErr.Error())
						return http.StatusInternalServerError, nil, defErr.Error(common.CCErrTopoHasHostCheckFailed)
					}
...
func (cli *instAction) getRawInstAsst(req *restful.Request, ownerID, objID string, ids []string, page map[string]interface{}, detail bool) ([]instNameAsst, int, int) {
	...
	case common.BKInnerObjIDIdc:
		...
		instName = common.BKIdcNameField
		instID = common.BKIdcIDField
		searchParams["sort"] = common.BKIdcIDField
		condition[common.BKOwnerIDField] = ownerID
		if 0 != len(tmpIDS) {
			condition[common.BKIdcIDField] = map[string]interface{}{common.BKDBIN: tmpIDS}
		}
	case common.BKInnerObjIDRack:
		targetOBJ = common.BKInnerObjIDRack
		instID = common.BKRackIDField
		instName = common.BKRackNameField
		searchParams["sort"] = common.BKRackIDField
		if 0 != len(tmpIDS) {
			condition[common.BKRackIDField] = map[string]interface{}{common.BKDBIN: tmpIDS}
		}
		condition[common.BKOwnerIDField] = ownerID
	case common.BKInnerObjIDPos:
		targetOBJ = common.BKInnerObjIDPos
		instID = common.BKPosIDField
		instName = common.BKPosNameField
		searchParams["sort"] = common.BKPosIDField
		condition[common.BKOwnerIDField] = ownerID
		if 0 != len(tmpIDS) {
			condition[common.BKPosIDField] = map[string]interface{}{common.BKDBIN: tmpIDS}
		}
	case common.BKInnerObjIDPlat:
...
func (cli *instAction) getInstDeteilByCondition(req *restful.Request, objID string, ownerID string, condition map[string]interface{}) (map[string]interface{}, int) {
	...
	case common.BKInnerObjIDHost:
		...
		targetpre = cli.CC.HostCtrl() + "/host/v1/hosts"
	case common.BKInnerObjIDPos:
		objType = common.BKInnerObjIDPos
		condition[common.BKOwnerIDField] = ownerID
	case common.BKInnerObjIDIdc:
		objType = common.BKInnerObjIDIdc
		condition[common.BKOwnerIDField] = ownerID
	case common.BKInnerObjIDRack:
		objType = common.BKInnerObjIDRack
		condition[common.BKOwnerIDField] = ownerID
	case common.BKInnerObjIDPlat:
...
func (cli *instAction) getInstDetail(req *restful.Request, instID int, objID, ownerID string) (interface{}, int) {
	...
	case common.BKInnerObjIDPos:
		...
		condition[common.BKOwnerIDField] = ownerID
	case common.BKInnerObjIDIdc:
		condition[common.BKIdcIDField] = instID
		condition[common.BKOwnerIDField] = ownerID
	case common.BKInnerObjIDRack:
