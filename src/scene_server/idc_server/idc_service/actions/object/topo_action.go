...
import (
	...
	"configcenter/src/scene_server/idc_server/idc_service/manager"
...
func (cli *topoAction) createDefaultInst(ownerID, objectID, objectName string, parentID int, req *restful.Request) (instID int, err error) {
	...
	case common.BKInnerObjIDIdc:
		targetOBJ = common.BKInnerObjIDIdc
		input[common.BKIdcNameField] = objectName
	case common.BKInnerObjIDPos:
		targetOBJ = common.BKInnerObjIDPos
		input[common.BKPosNameField] = objectName
	case common.BKInnerObjIDRack:
		targetOBJ = common.BKInnerObjIDRack
		input[common.BKRackNameField] = objectName
	default:
...
func (cli *topoAction) updateOldInstParentID(ownerID string, parentInstID int, cacheInst []manager.TopoInstRst, req *restful.Request) error {
	...
	for _, inst := range cacheInst {
		...
		case common.BKInnerObjIDIdc:
			targetOBJ = common.BKInnerObjIDIdc
			condition[common.BKIdcIDField] = inst.InstID
		case common.BKInnerObjIDPos:
			targetOBJ = common.BKInnerObjIDPos
			condition[common.BKPosIDField] = inst.InstID
		case common.BKInnerObjIDRack:
			targetOBJ = common.BKInnerObjIDRack
			condition[common.BKRackIDField] = inst.InstID
		default:
			targetOBJ = common.BKINnerObjIDObject
...
func (cli *topoAction) DeleteModel(req *restful.Request, resp *restful.Response) {
	...
	cli.CallResponseEx(func() (int, interface{}, error) {
		...
		switch objID {
		case common.BKInnerObjIDIdc, common.BKInnerObjIDHost, common.BKInnerObjIDPlat,
			common.BKInnerObjIDPos, common.BKInnerObjIDRack, common.BKInnerObjIDProc:
...
func (cli *topoAction) SelectModel(req *restful.Request, resp *restful.Response) {
	...
	cli.CallResponseEx(func() (int, interface{}, error) {
		...
		// deal data
		result, ctrErr := cli.mgr.SelectTopoModel(forward, nil, ownerID, common.BKInnerObjIDIdc, "", "", "", defErr)
		if nil != ctrErr {
...
func (cli *topoAction) selectInstDetail(req *restful.Request, ownerID, objID string, appID, instID, parentID int) (*ObjInstRsp, error) {
	...
	case common.BKInnerObjIDIdc:
		targetobj = common.BKInnerObjIDIdc
		searchParams["sort"] = common.BKIdcNameField
		if 0 != appID {
			condition[common.BKIdcIDField] = appID
		}
	case common.BKInnerObjIDRack:
		targetobj = common.BKInnerObjIDRack
		searchParams["sort"] = common.BKRackNameField
		if 0 != parentID {
			condition[common.BKIdcIDField] = parentID
		}
		if 0 != instID {
			condition[common.BKRackIDField] = instID
		}
	...
	case common.BKInnerObjIDPos:
		targetobj = common.BKInnerObjIDPos
		searchParams["sort"] = common.BKPosNameField
		if 0 != parentID {
			condition[common.BKRackIDField] = parentID
		}
		if 0 != instID {
			condition[common.BKPosIDField] = instID
		}
	default:
		targetobj = common.BKINnerObjIDObject
...
func (cli *topoAction) selectChildInstTopo(req *restful.Request, topoItem []manager.TopoModelRsp, level, appID, parentID int, childs *[]manager.TopoInstRst) error {
	...
	for _, childInstItem := range childInst.Data.Info {
		...
		case common.BKInnerObjIDIdc:
			if id, ok := childInstItem[common.BKIdcIDField]; ok {
				childInstRst.InstID = int(id.(float64))
			}
			if name, ok := childInstItem[common.BKIdcNameField]; ok {
				childInstRst.InstName = name.(string)
			}
		case common.BKInnerObjIDPos:
			if id, ok := childInstItem[common.BKPosIDField]; ok {
				childInstRst.InstID = int(id.(float64))
			}
			if name, ok := childInstItem[common.BKPosNameField]; ok {
				childInstRst.InstName = name.(string)
			}
		case common.BKInnerObjIDRack:
			if id, ok := childInstItem[common.BKRackIDField]; ok {
				childInstRst.InstID = int(id.(float64))
			}
			if name, ok := childInstItem[common.BKRackNameField]; ok {
				childInstRst.InstName = name.(string)
			}
		default:
...
func (cli *topoAction) SelectInstTopo(forward *api.ForwardParam, ownerID, objID string, appID, instID, level int, req *restful.Request) ([]manager.TopoInstRst, error) {
	...
	blog.Info("select module for insts:%v", rstItems)
	...
	for _, instItem := range inst.Data.Info {
		...
		case common.BKInnerObjIDIdc:
			if id, ok := instItem[common.BKIdcIDField]; ok {
				instRst.InstID = int(id.(float64))
			}
			if name, ok := instItem[common.BKIdcNameField]; ok {
				instRst.InstName = name.(string)
			}
		case common.BKInnerObjIDPos:
			if id, ok := instItem[common.BKPosIDField]; ok {
				instRst.InstID = int(id.(float64))
			}
			if name, ok := instItem[common.BKPosNameField]; ok {
				instRst.InstName = name.(string)
			}
		case common.BKInnerObjIDRack:
			if id, ok := instItem[common.BKRackIDField]; ok {
				instRst.InstID = int(id.(float64))
			}
			if name, ok := instItem[common.BKRackNameField]; ok {
				instRst.InstName = name.(string)
			}
		default:
...
func (cli *topoAction) SelectInst(req *restful.Request, resp *restful.Response) {
	...
	cli.CallResponseEx(func() (int, interface{}, error) {
		...
		rst, rstErr := cli.SelectInstTopo(forward, ownerID, common.BKInnerObjIDIdc, appID, 0, iLevel, req)
		if nil != rstErr {
